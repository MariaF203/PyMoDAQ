...
params_test = [
        {'title': 'Groups:', 'name': 'groups', 'type': 'group', 'children': [
            ...
            {'title': 'An expandable group:', 'name': 'cgroup', 'type': 'group', 'addText': 'Add', 'addMenu':
        ...
        # Test for color added here
        {'title': 'Booleans:', 'name': 'booleans', 'type': 'group', 'children': [
            {'title': 'A Color', 'name': 'acolor', 'type': 'color', 'value': QColor(255, 255, 255)},
...
settings = Parameter.create(name='settings', type='group', children=simple_params_test)
xml_string = ioxml.parameter_to_xml_string(settings)
xml_file = Path('xml_file.xml')

# Test with XML string in entry and an H5 file in output
converter = ParamH5Converter(xml_string)
h5_file = Path('converter_xml_string_test.h5')
converter.parameter_to_h5(h5_file)

# Test with XML file in entry and an H5 file in output
"""
converter = ParamH5Converter(xml_file)
h5_file = Path('converter_xml_file_test.h5')
converter.parameter_to_h5(h5_file)
"""

# Test with XML string in entry and an GROUP in output
"""
converter = ParamH5Converter(xml_string)
h5_file = Path('converter_group_test.h5')
saver_out = H5SaverLowLevel()
saver_out.init_file(h5_file)
group = saver_out.get_set_group('/RawData', 'test_save')
converter.parameter_to_h5(group)
"""
